<?php
// ============================================================
//  Short link redirect — public endpoint (no login).
//  Looks up a short code, logs the click, and redirects.
//  URL: s.php?c=CODE
//  (Mirrors r.php's dynamic-QR redirect pattern — see that file.)
// ============================================================
...
$st = db()->prepare("SELECT id, target_url, is_active, expires_at FROM short_links WHERE code = ? LIMIT 1");
$st->execute([$code]);
$r = $st->fetch();
...
try {
    db()->prepare("UPDATE short_links SET clicks = clicks + 1 WHERE id = ?")->execute([$r['id']]);
    $ipHash  = hash('sha256', ($_SERVER['REMOTE_ADDR'] ?? '') . '|qrstudio');
    $referer = mb_substr($_SERVER['HTTP_REFERER'] ?? '', 0, 500);
    $ua      = mb_substr($_SERVER['HTTP_USER_AGENT'] ?? '', 0, 255);
    db()->prepare("INSERT INTO short_clicks (link_id, ip_hash, referer, ua) VALUES (?,?,?,?)")
        ->execute([$r['id'], $ipHash, $referer, $ua]);
